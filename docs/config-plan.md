...
| S6    | `infr-rocm` (13) + `infr-metal` (20) on `Config`: `new_with(cfg)` both, S2's ROCm `load_from_env` bridge deleted, `INFR_PAGER_STATS` closed, Metal polarity tables written (compiler + macOS CI are its only gates). Both crates `INFR_*`-free                                                                                | `a2cffc7` |
...
### Carried into S7/S8 from S6

**12 knobs remain unmigrated tree-wide** (everything else is on `Config`):

- `device.ctx` — `crates/infr-llama/src/chat/{mod,diffusion}.rs`
- `sampling.{temp,top_k,top_p,seed,max_new,ignore_eos}` — the CLI's
  `publish_transitional_env` + `apply_model_sampling_defaults` bridge (**S8**)
- `sampling.no_think`, `debug.chat` — `infr-chat` (**S7**)
- `prof.profile_out` — `infr-prof-rt` (**S7**)
- `serve.{api_key,max_tokens_cap}` — `infr-server` (**S7**)

Plus the §6.10 permanent exclusions (`INFR_PROFILE` in five `build.rs`,
`INFR_TEST_GGUF`, `INFR_TEST_MODEL`, `INFR_LLAMA_DIFFUSION_CLI`,
`INFR_DIFFUSION_VISUAL`).

**`Config::load_from_env()` callers left:** `SeamModel::load` (S8) and
`CpuBackend::new`/`reference` as their env twins. `VulkanBackend::new` /
`RocmBackend::new` use the FALLIBLE `ConfigLayer::env()?` twin instead, so a bad
`INFR_SG`/`INFR_SUBMIT_DISPATCHES` still errors for a library caller.

**Note for anyone grepping the manifest:** it is generated by a `knobs!` macro
whose flag column is a bare ident, so `rg 'pending;'` finds the unmigrated keys
— `rg 'migrated: false'` returns nothing.
